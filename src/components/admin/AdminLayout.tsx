...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  const menuItems = [
    ...
    {
      id: 'game_control',
      label: 'WinGo Result Control (रिजल्ट सेटिंग)',
      icon: Sliders,
      highlight: true,
      badge: null,
    },
  ...
  return (
    <div className="min-h-screen admin-layout-root bg-[#0b0c10] text-[#e2e8f0] flex flex-col font-sans selection:bg-[#6366f1] selection:text-white">
      {/* Main Container */}
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto py-3 px-3 space-y-1 custom-scrollbar">
            {menuItems.map((item: any) => {
              ...
              const isHighlighted = item.id === 'game_control' || item.highlight;
              return (
                <button
                  key={item.id}
                  ...
                  className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs transition-all group ${
                    isHighlighted
                      ? isActive
                        ? 'bg-gradient-to-r from-amber-500 to-yellow-400 text-black font-extrabold shadow-xl shadow-amber-500/40 border border-yellow-300 ring-2 ring-amber-400/50'
                        : 'bg-gradient-to-r from-amber-500/20 via-yellow-500/15 to-amber-600/20 border-2 border-amber-400/70 text-amber-300 font-bold shadow-md shadow-amber-500/10 hover:border-amber-300 hover:bg-amber-500/30'
                      : isActive
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <Icon className={`w-4 h-4 flex-shrink-0 transition-transform ${isHighlighted ? (isActive ? 'text-black' : 'text-amber-400 animate-pulse') : isActive ? 'text-white' : 'text-slate-400 group-hover:text-indigo-400'}`} />
                    <span className="truncate">{item.label}</span>
                  </div>
                  {isHighlighted && !isActive && (
                    <span className="px-1.5 py-0.5 rounded text-[9px] font-black bg-amber-400 text-black uppercase tracking-wider animate-pulse">
                      RESULT SETTING
                    </span>
                  )}
                  {item.badge && (
        ...
        <div className="flex-1 flex flex-col lg:pl-64 min-w-0 overflow-x-hidden">
          ...
          <header className="h-16 bg-[#0d0e15]/90 backdrop-blur-md border-b border-[#1e202e] px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 shadow-md">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              {/* Dark Mode Indicator (Permanent) */}
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-[#131522] border border-[#23273c] text-xs font-semibold text-purple-300">
                <Moon className="w-3.5 h-3.5 text-purple-400" />
                <span className="hidden md:inline text-[11px] font-bold">Dark Mode</span>
              </div>
